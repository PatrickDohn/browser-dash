...
export default function SignInButton() {
  ...
  if (status === "loading") {
    return <div className="h-10 w-40 animate-pulse rounded-lg bg-gray-200" />
  }
    ...
    return (
      <div>
        <div className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white px-3 py-2 shadow-sm">
          ...
          <button
            onClick={() => signOut()}
            className="ml-2 text-sm font-medium text-gray-500 transition-colors hover:text-gray-900"
          >
            Sign out
          </button>
        </div>
